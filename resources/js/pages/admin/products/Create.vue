...
const taxableAmount = ref<number | undefined>(undefined);
const taxType = ref('GST 18%');
const taxPercentage = ref(props.gstSlabs['GST 18%'] ?? 0);
const lastEdited = ref<'price' | 'taxable'>('price');

const round2 = (value: number) => Math.round(value * 100) / 100;

// price = taxable × (1 + rate/100); recompute the derived side on any change.

...
import { create, index, store } from '@/routes/districts/locations';
import { index as districtsIndex } from '@/routes/states/districts';
import type { DistrictWithState } from '@/types';
...
defineOptions({
    layout: (props: { district: DistrictWithState }) => ({
        breadcrumbs: [
            ...
            { title: 'New location', href: create(props.district.id) },
        ],
    }),
});
</script>
...
<template>
    ...
    <div class="flex flex-col space-y-6">
        <Heading
            variant="small"
            title="New location"
            :description="`Create a new location in ${district.name}`"
        />

        <Form
            v-bind="store.form(props.district.id)"
